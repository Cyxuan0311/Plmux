...
from plmux.debug_log import PerfTimer, dbg, get_frame_profiler, perf_dbg, setup_debug_logging
from plmux.terminal._c_extension import _fastscreen
from plmux.extensions.registry import ExtensionContext, emit_hook, load_plugins, get_plugin_status_items
...
async def async_main(
    ...
    *,
        ...
        while ctx.running:
            ...
            try:
                await asyncio.to_thread(
                    emit_hook,
                    "status_refresh",
                    ExtensionContext(hook_name="status_refresh"),
                )
                ctx.dirty = True
            except Exception:
                pass
    ...
    if attach_mode:
        state, fds = connect_and_receive_sync()
        ...
        ctx.ws = PaneWorkspace(cfg, theme, on_dirty=ctx.mark_dirty, restore=snap)
        if snap is not None:
                ...
                while ctx.running:
                    ...
                    if ctx.dirty or keys:
                        ...
                        if ctx.broadcast_enabled:
                            extra_items.append(("BCAST", ctx.ws.theme.status_pane_style))
                        if getattr(ctx.ws, "zoom_pane", None) is not None:
                            extra_items.append(("ZOOM", ctx.ws.theme.status_win_style))
                        if ctx.mode == "copy":
                            extra_items.append(("COPY", ctx.ws.theme.status_pane_style))
                    ...
                    total_ms = frame_timer.elapsed_ms()
                    profiler.end_frame({
                        "input": input_ms,
                        "pty": pty_ms,
                        "render": render_ms,
